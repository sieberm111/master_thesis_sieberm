...
class FutEnv(Env):
    ...
    def transform_coordinates_out(self, list_coord):
        ...
        list_out[2] = (list_coord[2] * 0.082631579) - 0.785  # mapuje rozsah na -1.57:1.57 golie rotation
        list_out[3] = (list_coord[3] * 0.082631579) - 0.785  # mapuje rozsah na -1.57:1.57 def rotation
        return list_out
...
if __name__ == '__main__':
    ...
    wandb.init(project="master_thesis", entity="sieberm", name="PPO_multi_shot_small_rot2")
    wandb.config = {
        "learning_rate": 0.003,
        "time_steps": 500000,
        "batch_size": 64
    ...
    checkpoint_callback = CheckpointCallback(save_freq=100000, save_path='./saved_models_smal_rot/',
                                             name_prefix='rl_model_small_rot2')

    #model init
    #model = PPO("MlpPolicy", env, verbose=1)
    model = PPO.load("./saved_models_smal_rot/rl_model_small_rot_300000_steps.zip", env, verbose=1)

    # Train the agent
    model.learn(total_timesteps=500_000, callback=checkpoint_callback)
    # Save the agent
    model.save("saved_models_smal_rot/PPO_multishot_rot_small")
